const correlation = (function () {
    ...
    const generatecorrelationVector = function () {
        return new Promise(resolve => {
            const promises = [];
            const correlation = {};
                    ...
                    promises.push(
                        new Promise(resolve => {
                            defaultAttributeToFunction[attribute]().then(val => {
                                correlation[attribute] = val;
                                return resolve();
                            });
                        })
                    );
                } else {
                    correlation[attribute] = defaultAttributeToFunction[attribute]();
                }
            });
            return Promise.all(promises).then(() => {
                return resolve(correlation);
            });
        });
    };
    ...
    return {
        generatecorrelationVector: generatecorrelationVector,
        init: init,
        waitInit: waitInit
    };
})();
